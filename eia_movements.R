...
series <-subs$Series_IDs %>% filter(f=="M")
#us imports so that you can convert to volumes


series$name<-as.character(series$name)


api_data<-EIA_to_DF(series)
...
#EIA Imported Crude Universe
#API ID 1292190

series<-get_children(cat=1293019)%>% filter(f=="M")
series<-rbind(series,get_children(cat=1293182)%>% filter(f=="M"))
series<-series %>%arrange(series_id)
import_data_raw<-EIA_to_DF(series)#test with a small version use head(x,30)

import_data<-import_data_raw %>% melt(id="date")
#split variable into description
import_data$variable<-as.character(import_data$variable)
split<-do.call("rbind",strsplit(import_data$variable, "Imports of "))[,2]
import_data$grade<-do.call("rbind",strsplit(split, " from"))[,1]
split<-do.call("rbind",strsplit(import_data$variable, "from "))[,2]
import_data$From<-do.call("rbind",strsplit(split, " to"))[,1]
split<-do.call("rbind",strsplit(import_data$variable, "to "))[,2]
import_data$Dest<-do.call("rbind",strsplit(split, " in"))[,1]
split<-do.call("rbind",strsplit(import_data$variable, "in "))[,2]
import_data$To<-do.call("rbind",strsplit(split, ", Monthly"))[,1]

#Find countries that have been in US top 10
top_countries <- import_data %>%filter (date>ymd("2009-01-01")) %>% na.omit()%>% mutate(
  From=as_factor(From),
  To=as_factor(To),
  Dest=as_factor(Dest),
  grade=as_factor(grade)
)%>% filter(From != "Canada (Region)")%>% 
  filter(From != "Middle East")%>% 
  filter(From != "OPEC")%>% 
  filter(From != "Non-OPEC")%>% 
  filter(From != "Africa")%>% 
  filter(From != "Eurasia") %>%
  filter(From != "Asia-Pacific") %>%
  filter(From != "Europe") %>%
  filter(From != "Other Americas") %>%
  group_by(date,Dest,To) %>% mutate(share=value/sum(value*(From!="World"),na.rm = T))%>%
  ungroup()%>%
  filter(Dest=="refineries",grade=="all grades of crude oil",To!="PADD6 (Territories)",From!="World")%>%
  #filter(Dest=="refineries",To!="PADD6 (Territories)",From!="World")%>%
  group_by(From) %>% summarize(max=max(share,na.rm = T))%>%
  arrange(-max) %>% filter(max>=max[7])



#Do North America and Others First

import_data_clip <- import_data %>% mutate(
  From=as_factor(From),
  To=as_factor(To),
  Dest=as_factor(Dest),
  grade=as_factor(grade)
  ...
  filter(From != "Europe") %>%
  filter(From != "Other Americas") %>%
  mutate(cat=fct_other(From,keep = c("Canada","Mexico"),  other_level = "Other Countries")) %>%
  group_by(date,grade,Dest,To,cat) %>% summarize(value=sum(value,na.rm = T))%>%
  ungroup() %>%
  filter(grade!="all grades of crude oil") %>%
  mutate(combo=interaction(To,grade,sep=", "))

for(padd in levels(import_data_clip$To)){
file_name<-paste("imports_",make_clean_names(padd),".png")  
p<-ggplot(filter(import_data_clip,Dest=="refineries",To==padd))+
  #geom_line(aes(date,value,group=cat,color=cat))+
  geom_area(aes(date,value,colour=cat,fill=cat),position = "stack")+
  facet_grid( ~ grade)+
...
import_data_top <- import_data %>% mutate(
  From=as_factor(From),
  To=as_factor(To),
  Dest=as_factor(Dest),
  grade=as_factor(grade)
  ...
  filter(From != "Europe") %>%
  filter(From != "Other Americas") %>%
  mutate(cat=fct_other(From,keep = as.character(top_countries$From),  other_level = "Other Countries")) %>%
  group_by(date,grade,Dest,To,cat) %>% summarize(value=sum(value,na.rm = T))%>%
  ungroup() %>%
  filter(grade!="all grades of crude oil",To!="PADD6 (Territories)") %>%
  filter(To!="PADD6 (Territories)") %>% 
  mutate(To=factor(To, levels=c("PADD1 (East Coast)","PADD2 (Midwest)","PADD3 (Gulf Coast)","PADD4 (Rocky Mountain)",
                                "PADD5 (West Coast)")),
        grade=factor(grade,levels = c("all grades of crude oil","light sweet crude oil"  ,"light sour crude oil",    "medium crude oil"        ,
       "heavy sweet crude oil","heavy sour crude oil")),
       grade_trim=fct_collapse(grade,
                               "light crude oil"=c("light sweet crude oil"  ,"light sour crude oil"),
                                 "heavy crude oil"=c("heavy sweet crude oil","heavy sour crude oil"))
       
  )%>%group_by(date,grade_trim,Dest,To,cat) %>% summarize(value=sum(value,na.rm = T))%>% ungroup

levels(import_data_top$grade_trim) <- stri_trans_totitle(levels(import_data_top$grade_trim))

for(padd in levels(import_data_top$To)){
  file_name<-paste("imports_",make_clean_names(padd),".png")  
  p<-ggplot(filter(import_data_top,Dest=="refineries",To==padd))+
    #geom_line(aes(date,value,group=cat,color=cat))+
    geom_area(aes(date,value,colour=cat,fill=cat),position = "stack")+
    facet_grid( ~ grade)+
    #geom_col(aes(Prov,prov_ghgs/10^6,colour=Sector,fill=Sector,group=Ref_Year),size=1.5,position = "dodge")+
    ...
    #scale_fill_viridis("",discrete=TRUE)+
    scale_y_continuous()+
    scale_fill_manual("",values = colors_tableau10()[-7],guide = "legend",drop=F)+
    scale_colour_manual("",values=colors_tableau10()[-7],guide = "legend",drop=F)+
    guides(fill=guide_legend(nrow =2,byrow=FALSE))+
    theme_bw()+theme(
      plot.margin = margin(.5, .5, .5, .5, "cm"),
      legend.position = "bottom",
  ...
  labs(x=NULL,y="Monthly imports (mmbbl/d)",
       #title=paste("2009-2019 Imports by Grade",padd,sep=", "),
       title=paste(padd),
       caption="Source: EIA data, graph by @andrew_leach")
  ggsave(file_name,p)
}


p2<-ggplot(filter(import_data_top,Dest=="refineries"))+
  #geom_line(aes(date,value,group=cat,color=cat))+
  geom_area(aes(date,value/days_in_month(date)/1000,colour=cat,fill=cat),position = "stack")+
  facet_grid(grade_trim~To)+
  #geom_col(aes(Prov,prov_ghgs/10^6,colour=Sector,fill=Sector,group=Ref_Year),size=1.5,position = "dodge")+
  #scale_color_viridis("",discrete=TRUE,guide_legend(NULL))+
  #scale_fill_viridis("",discrete=TRUE)+
  ...
  theme_bw()+theme(
    ...
    axis.text.x = element_text(size = 11, colour = "black",hjust = .5),
    strip.text = element_text(size = 11, colour = "black", angle = 0),
    axis.title.y = element_text(margin = margin(r = 5))
    
  )+
  labs(x=NULL,y="Monthly Imports, Million Barrels per Day (mmbbl/d)",
       title=paste("2009-2019 Imports by Crude Oil Grade and PADD"),
       caption="Source: EIA data, graph by @andrew_leach")

ggsave("imports_grade_padd.png",p2)


p2<-ggplot(filter(import_data_top,Dest=="refineries",cat!="Canada"))+
  #geom_line(aes(date,value,group=cat,color=cat))+
  geom_area(aes(date,value/days_in_month(date)/1000,colour=cat,fill=cat),position = "stack")+
  facet_grid(grade_trim~To)+
  #geom_col(aes(Prov,prov_ghgs/10^6,colour=Sector,fill=Sector,group=Ref_Year),size=1.5,position = "dodge")+
...
remaining_mkt<-import_data_top%>%filter(Dest=="refineries",cat!="Canada") %>% group_by(date,To) %>% summarize(value=sum(value,na.rm = T)/mean(days_in_month(date)))
